...
// Programming subjects with specific questions
...
// Function to generate questions for programming subjects
function generateProgrammingQuestions() {
  const allQuestions = [];
  
  for (const [subject, difficulties] of Object.entries(programmingSubjects)) {
...
// Function to seed programming quiz questions
export async function seedProgrammingQuizQuestions() {
  console.log('Seeding programming quiz questions...');
  ...
  for (const questionData of questions) {
    await prisma.question.create({
      data: {
        id: `${questionData.subject.toLowerCase().replace(/\s+/g, '-')}-${questionData.difficulty}-${Math.random().toString(36).substr(2, 9)}`,
        question: questionData.question,
        options: questionData.options,
        correctAnswer: questionData.correctAnswer,
        explanation: questionData.explanation,
        category: questionData.subject,
  ...
  console.log(`Successfully seeded ${totalQuestions} programming quiz questions!`);
}

// Function to run the seeding
async function main() {
  try {
    await seedProgrammingQuizQuestions();
  } catch (error) {
    console.error('Error seeding programming quiz questions:', error);
  } finally {

...
import type { UnifiedRecommendation, LearningResource } from '@/lib/unified-recommendation-engine'

interface UnifiedRecommendationPanelProps {
  recommendations: UnifiedRecommendation
  onStartQuiz?: (category: string, difficulty: string) => void
  onViewResource?: (resource: LearningResource) => void
...
export function UnifiedRecommendationPanel({
  ...
  const handleTakeRecommendedQuiz = () => {
    if (onStartQuiz) {
      onStartQuiz(nextQuizSuggestion.category, nextQuizSuggestion.difficulty)
    } else {
      router.push(`/quiz/new?category=${nextQuizSuggestion.category}&difficulty=${nextQuizSuggestion.difficulty}`)
    }
  }

  // If advanced features are disabled, show basic view
      ...
      <div className={`space-y-6 ${className}`}>
        ...
        {performanceAnalytics && (
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                {getTrendIcon(performanceAnalytics.learningTrend)}
            ...
            <CardContent>
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="text-2xl font-bold">{Math.round(performanceAnalytics.overallScore)}%</p>
                </div>
                <div>
                  <p className="text-muted-foreground">Learning Trend</p>
                  <p className="text-2xl font-bold capitalize">{performanceAnalytics.learningTrend}</p>
                </div>
              </div>
              <p className="text-sm text-muted-foreground mt-2">
                {performanceAnalytics.learningTrend === "improving" && "Great job! You're showing consistent improvement."}
                {performanceAnalytics.learningTrend === "declining" && "Consider reviewing fundamentals to get back on track."}
                {performanceAnalytics.learningTrend === "stable" && "You're maintaining steady performance."}
              </p>
            </CardContent>
          </Card>
        )}
          ...
          <CardContent>
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                <Badge className="capitalize">{nextQuizSuggestion.category}</Badge>
                <Badge variant="outline" className="capitalize">
                  {nextQuizSuggestion.difficulty}
                </Badge>
              </div>
              <p className="text-sm text-muted-foreground">{nextQuizSuggestion.reason}</p>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="overview" className="space-y-4">
            <OverviewTab 
              ...
              onStartQuiz={onStartQuiz || handleTakeRecommendedQuiz}
            />
          </TabsContent>
          ...
          <TabsContent value="resources" className="space-y-4">
            <ResourcesTab 
              ...
              onViewResource={onViewResource || handleResourceClick}
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          {recommendedResources.length > 0 ? (
            recommendedResources.map((resource: LearningResource, index: number) => (
